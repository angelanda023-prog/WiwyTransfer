...
// Clase base de la conexión Nearby/Quick Share: framing, UKEY2 y canal seguro.
...
class NearbyConnection {
    ...
    private static let dispatchQueue = DispatchQueue(label: "com.wiwy.wiwytransfer.qs.queue", qos: .utility)
    ...
    // Claves del canal seguro (SecureMessage)
    ...
    func start() {
        connection.stateUpdateHandler = { state in
            if case .ready = state {
                ...
                self.lastError = err
                print("QS: error abriendo socket: \(err)")
    ...
    private func receiveFrameAsync() {
        connection.receive(minimumIncompleteLength: 4, maximumLength: 4) { content, _, isComplete, error in
            ...
            if isComplete { self.handleConnectionClosure(); return }
            if error != nil { self.lastError = error; self.protocolError(); return }
            ...
            guard frameLength < NearbyConnection.SANE_FRAME_LENGTH else {
                self.lastError = NearbyError.protocolError("Longitud de paquete inesperada")
                self.protocolError()
                return
            }
    ...
    private func receiveFrameAsync(length: UInt32) {
        connection.receive(minimumIncompleteLength: Int(length), maximumLength: Int(length)) { content, _, isComplete, _ in
            if self.connectionClosed { return }
            if isComplete { self.handleConnectionClosure(); return }
            guard let content = content else { self.protocolError(); return }
    ...
    internal func decryptAndProcessReceivedSecureMessage(_ smsg: Securemessage_SecureMessage) throws {
        ...
        clientSeq += 1
        guard d2dMsg.sequenceNumber == clientSeq else { throw NearbyError.protocolError("Número de secuencia incorrecto. Esperado \(clientSeq), recibido \(d2dMsg.sequenceNumber)") }
        ...
        if offlineFrame.hasV1 && offlineFrame.v1.hasType, case .payloadTransfer = offlineFrame.v1.type {
            ...
            if case .bytes = header.type {
                ...
                if header.totalSize > NearbyConnection.SANE_FRAME_LENGTH {
                    ...
                    throw NearbyError.protocolError("Payload demasiado grande (\(header.totalSize) bytes)")
                }
                ...
                guard chunk.offset == buffer.count else {
                    payloadBuffers.removeValue(forKey: payloadID)
                    throw NearbyError.protocolError("Offset de chunk inesperado \(chunk.offset), esperado \(buffer.count)")
                }
                if chunk.hasBody {
            ...
            print("QS: offline frame cifrado no manejado: \(offlineFrame)")
        }
    }
    ...
    internal static func hkdf(inputKeyMaterial: SymmetricKey, salt: Data, info: Data, outputByteCount: Int) -> SymmetricKey {
        HKDF<SHA256>.deriveKey(inputKeyMaterial: inputKeyMaterial, salt: salt, info: info, outputByteCount: outputByteCount)
    }
    ...
    internal func sendKeepAlive(ack: Bool) {
        ...
        do {
            ...
            print("QS: error enviando KEEP_ALIVE: \(error)")
        }
    }
}
